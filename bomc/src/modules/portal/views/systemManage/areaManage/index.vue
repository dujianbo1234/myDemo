						...
						<el-col>
							<el-card>
										...
										<el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline">
											<el-form-item label="地域名字" value="地域名字" prop="areaName">
												<el-input v-model="formInline.areaName" style="width: 210px;"></el-input>
											</el-form-item>
											<el-form-item label="地域状态" prop="status">
...
<el-button type="primary" icon='document' @click="exportLogo()"><i class="el-icon-download"></i>导出excel</el-button>
					...
					<el-row>
						<el-col>
							<el-card>
								...
								<el-table :data="tableData.slice((currentPage-1)*pagesize,currentPage*pagesize)" :default-sort="{prop: 'createTime', order: 'descending'}" @selection-change="handleSelectionChange" @select-all="selectAllRus">
									<el-table-column type="selection" width="55">
									</el-table-column>
									<el-table-column prop="areaId" label='地域ID' value="地域ID" width="80">
									</el-table-column>
									<el-table-column prop="areaName" label='地域名字' value="地域名字" width="130">
									</el-table-column>
									<el-table-column prop="createTime" resizable sortable label='创建时间' value="创建时间" width="170">
									</el-table-column>
									<el-table-column prop="remark" label='描述' value="描述" min-width="180">
									</el-table-column>
	...
	<el-table-column label='操作管理' fixed="right" width="80">
		...
		<i class="iconfont co" title="禁用"  @click="handleDelete(scope.row.areaId,1)">&#xe65a;</i> 
		<i class="iconfont co" title="恢复" @click="beback(scope.row.areaId)">&#xe65c;</i>
								</template>
		</el-table-column>
	    ...
	    <el-button class="form_btn" size="mini"   @click="handleDelete(scope.row.areaId,2)">删除</el-button>
		...
		<!--新增 -->
		<el-dialog title="新增地域" :visible.sync="addUserDialog" :before-close="handleClose">
			<el-form :model="formAdd" :rules="rules" ref="formAdd" style="margin:10px;width:auto;">

				<el-form-item label="地域名字:" :label-width="formLabelWidth" prop="areaName" required>
					<el-input v-model.trim="formAdd.areaName" auto-complete="off"></el-input>
				</el-form-item>
		...
		<!--03-编辑-->
		<el-dialog title="编辑地域" :visible.sync="resetDialog22" :before-close="handleClose">
			<el-form :model="formEd" :rules="rules" ref="formEd" style="margin:10px;width:auto;">

				<el-form-item label="地域名字:" :label-width="formLabelWidth" prop="areaName" required>
					<el-input v-model.trim="formEd.areaName" auto-complete="off"></el-input>
				</el-form-item>
				...
				<!--<el-dialog title="编辑组织" :visible.sync="resetDialog22">
			<el-form :model="formEd" :rules="rules" ref="formEd" style="margin:10px;width:auto;">
...
<script>
	...
	import { roleSch, rightDele, adCmit, recover, editCmit } from "../../../api/areaManage";
	export default {
		data() {
			...
			return {
				...
				rules: {
					...
					areaName: [
						{ required: true, message: '请输入地域名字', trigger: 'blur' },
		...
		methods: {
			...
			handleEdit(row) {
				...
				this.formEd.areaId = row.areaId;
				this.formEd.remark = row.remark;
				this.formEd.areaName = row.areaName;
				this.formEd.areaKey = row.areaKey;
			},
			//编辑弹窗的提交按钮
