      		...
      		<div class="RightRow">
        		<el-card>
					<div class="buttonItem">
						<el-button size="small" plain class="opt_btn" @click="exportLogo(typeBol)">导出</el-button>
					</div>
					<el-table v-if="typeBol == '0' || typeBol == '5'"  :data="tableData" tooltip-effect="dark" @selection-change="handleSelectionChange" :header-cell-style="headStyle">
                    ...
                    <el-table v-else-if="typeBol == '10'" :data="tableData" tooltip-effect="dark" @selection-change="handleSelectionChange" :header-cell-style="headStyle" >
                        <el-table-column type="selection"  />
                        <el-table-column show-overflow-tooltip prop="name" align='center' label="命令名称" >
                        </el-table-column>
                        <el-table-column show-overflow-tooltip prop="platformName" align='center' label="平台类型" >
                        </el-table-column>
                        <el-table-column show-overflow-tooltip prop="resourceClassName" align='center' label="资源类型" >
                        </el-table-column>
                        <el-table-column show-overflow-tooltip prop="resourceTypeName" align='center' label="对象类型">
                        </el-table-column>
                        <el-table-column show-overflow-tooltip prop="targetHost" align='center' label="对象详情">
                        </el-table-column>
                        <el-table-column show-overflow-tooltip prop="createTime" align='center' label="操作时间">
                        </el-table-column>
                        <el-table-column prop="resolveResult" align='center' label="操作结果">
                        ...
                        <el-table-column show-overflow-tooltip prop="creator" align='center' label="操作人">
                        </el-table-column>
                    </el-table>
...
<script>
import * as service from '@/modules/ops/api/ops/journalApi/journalApi.js'
...
export default {
    ...
    methods: {
        ...
        exportLogo(typeBol) {
            ...
            console.log(typeBol)
            if(typeBol == '5'){
                tHeader = ['操作类型','操作项名称','操作前','操作后','操作原因','操作时间','操作人']
                filterVal = ['operationType','modifyTermName','beforeModify','afterModify','operateReason','updateTime','operateUserName']
                filename = '巡检日志'
            }else if(typeBol == '0'){
                tHeader = ['操作类型','操作项名称','操作前','操作后','操作原因','操作时间','操作人']
                filterVal = ['operationType','modifyTermName','beforeModify','afterModify','operateReason','updateTime','operateUserName']
                ...
                tHeader = ['命令名称','平台类型','资源类型','对象类型','对象详情','操作时间','操作结果','操作人']
                filterVal = ['name','platformName','resourceClassName','resourceTypeName','targetHost','createTime','center','creator']
                filename = '命令统计日志'
            }
        ...
        queryData() {
            ...
            if(this.typeBol == '0'){
                 service.queryDataCps(params).then(res => {
                    if(res.code == 200) {
            ...
            }else if(this.typeBol == '10'){
                service.queryCommand(params).then(res => {
                    if(res.code == 200) {
                        this.tableData = res.result.list
                        this.totalCount=res.result.count
                    }
