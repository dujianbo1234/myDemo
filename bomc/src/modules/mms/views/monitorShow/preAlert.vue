  ...
  <div class="monitor-container">
          ...
          >
            <el-row>
              <el-col :span="6">
                ...
                <el-input v-model="currentAlertSearchData.name" placeholder="请输入预警名称"></el-input>
                <!-- </el-form-item> -->
              </el-col>
              <el-col :span="6">
                <!-- <el-form-item label="告警编码" > -->
                <el-input v-model="currentAlertSearchData.code" placeholder="请输入预警编码"></el-input>
                <!-- </el-form-item> -->
              </el-col>
              <el-col :span="6">
                <!-- <el-form-item label="告警ID" > -->
                <el-input v-model="currentAlertSearchData.id" placeholder="请输入预警ID"></el-input>
                <!-- </el-form-item> -->
              </el-col>
              <el-col :span="6">
                <!-- <el-form-item label="监控对象"> -->
                <el-input v-model="currentAlertSearchData.resourceObject" placeholder="请输入监控对象"></el-input>
                <!-- </el-form-item> -->
              </el-col>
            </el-row>
            <br>
            <el-row>
              <el-col :span="6">
                ...
                <el-select v-model="currentAlertSearchData.level" placeholder="请选择预警级别">
                  <el-option
              ...
              <el-col :span="6">
                ...
                <el-select v-model="currentAlertSearchData.status" placeholder="请选择预警状态">
                  <el-option
              ...
              <el-col :span="6">
                ...
                <el-input v-model="currentAlertSearchData.content" placeholder="请输入预警内容"></el-input>
                <!-- </el-form-item> -->
              </el-col>
              <el-col :span="6">
            ...
            <br>
            <el-row>
              <!-- <el-col :span="4" id="alertStyle">
                <el-button type="primary" @click="alertDown=!alertDown">告警选择器</el-button>
                <div class="alert-info" v-if="alertDown">
                  <div class="title">
                    <i></i>告警选择器
                    <el-button
                      id="select-add"
                  ...
                  <el-form :model="alertSelertFrom" class="alertHStyle">
                    <el-form-item v-for="(domain,index) in alertSelertFrom.domains" :key="index">
                      ...
                      <el-input v-model="domain.value" class="value-length"></el-input>
                      <el-button
                        type="danger"
                        icon="el-icon-delete"
    ...
    <el-card  class="cardM">
      <el-row class="btnInfo">
        <el-col :span="18">
          <el-button type="primary" @click="currentAlertDispose">处理</el-button>
        </el-col>
        <el-col :span="6">
          <el-button type="primary" @click="currentAlertExportCheck">导出</el-button>
          <el-button type="primary" @click="currentAlertExportAll">导出所有</el-button>
      ...
      <div v-loading="listLoading">
        <el-table
          ...
          @row-click="currentAlertRowChange"
        >
          <el-table-column type="selection" width="55"></el-table-column>
          <el-table-column prop="name" label="预警名称" width="100"></el-table-column>
          <el-table-column prop="code" label="预警编码" width="80"></el-table-column>
          <el-table-column label="预警ID" width="210">
            <template slot-scope="scope">{{ scope.row.id }}</template>
          </el-table-column>
          <el-table-column v-if="currentAlertStateShow" label="预警状态" width="100">
            <template slot-scope="scope">{{ scope.row.status | leavelState}}</template>
          </el-table-column>
          <el-table-column v-if="currentAlertobjShow" prop="resourceObject" label="对象" width="90"></el-table-column>
          <el-table-column
          ...
          ></el-table-column>
          <el-table-column v-if="currentAlertLastTimeShow" label="最后发生时间">
            <template slot-scope="scope">{{scope.row.lastAlertTime | strToDataFromat}}</template>
          </el-table-column>

          <el-table-column v-if="currentAlertLeaveShow" label="当前预警级别" width="120">
            <template slot-scope="scope">
              <el-button type="primary" v-if="scope.row.level =='0'">一般预警</el-button>
              <el-button type="warning" v-if="scope.row.level =='1'">主要预警</el-button>
              <el-button type="danger" v-if="scope.row.level =='2'">严重预警</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
    ...
    <el-dialog :visible.sync="currentAlertDialog" width="70%" id="dialog">
      ...
      <div slot="footer" class="dialog-footer" align="center">
        <el-button @click="currentAlertDialog = false">返回</el-button>
      </div>
    </el-dialog>
    <!-- 当前告警表单设置显示隐藏 -->
    <el-dialog
    ...
    >
      ...
      <div slot="footer" class="dialog-footer" align="center">
        <el-button @click="confirmCurrentAlertShow" type="primary">确定</el-button>
      </div>
    </el-dialog>
    <!-- 当前告警处理按钮 -->
    <el-dialog
    ...
    >
      <div v-for="(item, index) in disposeAlertData" :key="index">
        <ul class="alertName">
           <li class="titlefont">
            <el-checkbox v-model="alertChecked[index]" @change="alertCheckChange(item.id,alertChecked[index])" :disabled="item.status==0?false:true"></el-checkbox>
            {{item.code}}
            <i class="el-icon-caret-bottom" @click="showTotal(index)"></i>
          </li>
          <li>{{item.appId}}</li>
          <li>{{item.status | leavelState}}</li>
          ...
          <div v-show="showArr[index].key1">
            ...
            >
              ...
              <el-table-column label="告警级别">
                <template slot-scope="scope">{{scope.row.level | alertLeavel}}</template>
              </el-table-column>
      ...
      <div slot="footer" class="dialog-footer" align="center">
        <el-button type="primary" @click="disposeConfirm(false)">确定</el-button>
        <el-button type="primary" @click="toOrderList">转工单</el-button>
        <el-button @click="currentAlertDisposeDialog = false">取消</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      ...
      <div slot="footer" class="dialog-footer" align="center">
        <el-button @click="confirmAlertHistroyShow" type="primary">确定</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      title="预警详情"
      :visible.sync="preAlertHistoryDialog"
      width="70%"
      id="preAlertHistoryDialog"
      >
      <el-tabs v-model="activeName">
        <el-tab-pane label="预警信息" name="first">
          <el-form :inline="true" label-position="right" label-width="100px" size="mini">
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="指标">活动预警</el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="8">
                <el-form-item label="策略ID">策略ID</el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="首次发生时间">2018-06-11 12:20:00</el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="最后发生时间">2018-06-11 12:20:00</el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="8">
                <el-form-item label="预警处理时间">2018-06-11 12:20:00</el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="预警清除时间">2018-06-11 12:20:00</el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="当前预警级别">主要预警</el-form-item>
              </el-col>
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="预警来源">threshold-checher</el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="预警状态">未确认</el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="8">
                <el-form-item label="处理人">李四</el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="预警标题">预警标题</el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="预警内容">告警内容</el-form-item>
              </el-col>
      ...
      <div slot="footer" class="dialog-footer" align="center">
        <el-button @click="preAlertHistoryDialog = false">返回</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    clearAlertData(){
      //处理的dialog消失之前,先把所选择的告警放到数组里面
       this.toOrderIdArr=this.alertCheckData.concat(this.currentAlertDisposeSelectDataMerge)
    ...
    disposeConfirm(order) {
      ...
      let data = {
        ...
        id: this.alertCheckData.concat(this.currentAlertDisposeSelectDataMerge)
          .concat(this.currentAlertDisposeSelectDataRelation)
          .join(","),
    ...
    toOrderList() {
      ...
      this.styleInfo=[];
      //请求工单信息
      if(!this.alertCheckData.length && !this.currentAlertDisposeSelectDataMerge.length && !this.currentAlertDisposeSelectDataRelation.length){
        this.$message({
          message: "请选择要转工单的告警",
          type: "error"
        });
...
.monitor-container .el-tabs__content {
  overflow: visible;
}
// .el-tooltip__popper {
//   display: none;
