  ...
  <div class="oms-diy-elementui eventContainer">
    ...
    >
      ...
      <div class="dealStepsArea">
        <process-steps :procInstId.sync="procInstId" @fullMsg="getFullMsg"></process-steps>
      </div>
      ...
      <el-tabs type="border-card" class="tabsArea">
        <el-tab-pane label="解决方案">
          <el-row>
            <el-col :span="24">
              <el-form-item class="dealSuggestion textarea" label="解决方案：" prop="fullMsg">
                <span class="inputTips">最多输入500个字符</span>
      ...
      <el-row class="operation">
        <el-button type="info">转紧急变更流程</el-button>
  ...
  methods: {
    ...
    queryData() {
      ...
      service.queryData(params).then(res => {
        ...
        this.ruleForm.eventStatus = "紧急事件处理";
        ...
        if (this.ruleForm.isRepeating === null) {
          ...
          this.ruleForm.isRepeating = this.ruleForm.isRepeating + "";
        }
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          ruleForm.isRepeating = parseFloat(ruleForm.isRepeating);
    ...
    nextNodeDealMan() {
      ...
      // });
      this.nextNodeDealManList = [{ name: "zhangxd", userId: 28 }];
    },
    querySystemType() {
      //查询系统类型信息
