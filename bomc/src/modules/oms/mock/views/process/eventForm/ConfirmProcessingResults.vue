    ...
    >
      ...
      <el-tabs type="border-card" class="tabsArea">
        <el-tab-pane label="解决方案">
          <el-row>
            <el-col :span="24">
              <el-form-item class="dealSuggestion" label="解决方案：">
                <el-input type="textarea" v-model="ruleForm.fullMsg" class="oms-input" disabled></el-input>
              </el-form-item>
            </el-col>
          </el-row>
        </el-tab-pane>
...
export default {
  name: "ConfirmProcessingResults", // 确认处理结果
  data() {
    return {
      userId: "", //用户id
      procInstId: "", // 流程实例id
  ...
  methods: {
    ...
    queryData() {
      ...
      service.queryData(params).then(res => {
        ...
        this.ruleForm.eventStatus = "紧急事件确认结果";
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.ruleForm.checkResult == 0) {
            //未解决
            data.variables = [{ key: "relieve", value: "false" }];
          } else {
            //已解决
            data.variables = [{ key: "relieve", value: "true" }];
          }
    ...
    getFullMsg(val) {
      console.log("this.ruleForm.fullMsg", this.ruleForm.fullMsg);
