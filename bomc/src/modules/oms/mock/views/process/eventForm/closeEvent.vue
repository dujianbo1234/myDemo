  ...
  <div class="oms-diy-elementui eventContainer">
    ...
    >
      ...
      <div class="basicInfoArea">
        ...
        <el-row>
          <el-col :span="16">
            <el-form-item class="eventDicr closeEvent" label="事件描述：" prop>
          ...
          <el-col :span="8">
            ...
            <el-form-item label="业务恢复时间：" prop="businessRecoveryTime" v-if="timeShow">
              <el-date-picker
                class="oms-input"
            ...
            <el-form-item label="业务恢复时间：" v-if="!timeShow">
              <el-date-picker
                class="oms-input"
                v-model="ruleForm.businessRecoveryTime"
                type="datetime"
            ...
            <el-form-item label="关联变更单号：">
              <el-input class="oms-input" v-model="ruleForm.relatedChangeId" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="重复事件标记：" prop>
              <el-select v-model="ruleForm.isRepeating" class="oms-select" :disabled="true">
      ...
      <div class="dealStepsArea">
        <el-row>
          <div class="isInterruptRow left">是否中断</div>
          <div class="isInterrupt">
            <el-radio v-model="isInterrupt" label="0" value="0">是</el-radio>
            <el-radio v-model="isInterrupt" label="1" value="1">否</el-radio>
          </div>
        </el-row>
        <el-row v-if="isInterrupt == 0">
          <el-col :span="8">
            <el-form-item label="综合账务中断时长" prop="interruption" class="duration">
      ...
      <div class="dealStepsArea">
        <el-row>
          <el-col :span="8">
            <el-form-item label="事件结束代码" prop="completeCode">
              <el-select v-model="ruleForm.completeCode" placeholder class="oms-select">
                <el-option label="成功解决" value="成功解决"/>
                <el-option label="变通方法解决" value="变通方法解决"/>
                <el-option label="不成功" value="不成功"/>
                <el-option label="消失" value="消失"/>
                <el-option label="误报" value="误报"/>
                <el-option label="可忽略" value="可忽略"/>
              </el-select>
            </el-form-item>
          </el-col>
          <!-- <el-col
            :span="8"
            v-if="ruleForm.completeCode==='成功解决'|| ruleForm.completeCode==='变通方法解决'"
          >
            <el-form-item
              label="事件解决人角色"
              prop="eventSolverRole"
            >
              <el-select
          ...
          <el-col
            ...
            v-if="ruleForm.completeCode==='成功解决'|| ruleForm.completeCode==='变通方法解决'"
          >
            <el-form-item label="事件解决人" prop="eventSolver">
              <el-input v-model="ruleForm.eventSolver" class="oms-input" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="处理是否超时" prop>
              <el-select v-model="ruleForm.isTimeout" class="oms-select" :disabled="true">
                <el-option label="已超时" :value="0"></el-option>
                <el-option label="未超时" :value="1"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="故障厂商" prop="manufacturerId">
              <el-input v-model="ruleForm.manufacturerId" class="oms-input"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      ...
      <el-tabs type="border-card" class="tabsArea">
        <el-tab-pane label="解决方案">
          <el-row>
            <el-col :span="24">
              <el-form-item class="dealSuggestion" label="解决方案：" prop>
                <el-input
                  type="textarea"
...
export default {
  ...
  computed: {
    ...
    timeDisabled() {
      //成功解决 或 变通方法解决 或 ''
      let isEdit =
        this.ruleForm.completeCode === "成功解决" ||
        this.ruleForm.completeCode === "变通方法解决" ||
        this.ruleForm.completeCode === "" ||
        this.ruleForm.completeCode === null;
      if (isEdit) {
        this.timeShow = true;
        return false;
      } else {
        if (this.ruleForm.businessRecoveryTime) {
          this.$alert(
            `由于事件结束代码为： ${
              this.ruleForm.completeCode
            }，业务恢复时间将被清空`,
        ...
        this.ruleForm.eventSolverRole = "";
        this.ruleForm.eventSolver = "";
        return true;
      }
    }
  },
  watch: {
    ...
    "ruleForm.completeCode": function(val) {
      if (val === "成功解决" || val === "变通方法解决") {
        this.querySolver();
      }
    },
    isInterrupt(val, oldVal) {
      if (val === "1") {
        this.ruleForm.interruption = "";
        this.ruleForm.convergentBillingInterruption = "";
        this.ruleForm.customerServiceInterruption = "";
      }
    }
  },
  ...
  methods: {
    ...
    queryData() {
      ...
      service.queryData(params).then(res => {
        ...
        this.ruleForm.eventStatus = "事件关闭";
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          //时长转化为number
          ruleForm.interruption = parseFloat(ruleForm.interruption);
          ruleForm.convergentBillingInterruption = parseFloat(
            ruleForm.convergentBillingInterruption
          );
          ruleForm.customerServiceInterruption = parseFloat(
            ruleForm.customerServiceInterruption
          );
          // if (this.isInterrupt === "1") {
          //   delete ruleForm.interruption
          //   delete ruleForm.convergentBillingInterruption
          //   delete ruleForm.customerServiceInterruption
          // }

          if (
            ruleForm.completeCode !== "成功解决" ||
            ruleForm.completeCode !== "变通方法解决"
          ) {
            delete ruleForm.businessRecoveryTime;
          }
    ...
    querySolver() {
      ...
      service.querySolver(params).then(res => {
        this.ruleForm.eventSolver = res.realName;
      });
    },
    ...
    validatePass(rule, value, callback) {
      ...
      if (
      ...
      } else if (rule.fullField === "businessRecoveryTime") {
        //验证业务恢复时间
        if (new Date(value).getTime() > Date.now()) {
          callback(new Error(" "));
          this.$notify({
            title: "提示",
            message: "业务恢复时间不能大于当前时间",
            duration: 2000
          });
        } else if (
          new Date(value).getTime() <
          new Date(this.ruleForm.eventTime).getTime()
        ) {
          callback(new Error(" "));
          this.$notify({
            title: "提示",
        ...
        rule.fullField === "eventSolver" ||
        rule.fullField === "manufacturerId"
      ) {
        if (value && !this.getStrLenght(value, 40)) {
          callback(new Error(" "));
          this.$notify({
            title: "提示",
      ...
      // }else if(new Date(value).getTime()< new Date(this.ruleForm.eventTime).getTime()){
      //   callback(new Error(" "));
      //   this.$notify({
      //     title: "提示",
      //     message: '业务恢复时间不能小于事件发生时间'
