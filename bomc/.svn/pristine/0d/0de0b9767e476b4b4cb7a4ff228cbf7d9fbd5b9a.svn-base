<template>
  <div class="oms-diy-elementui eventContainer problemContainer changeForm makePlan">
    ...
    >
          ...
          <el-col :span="8">
            <el-form-item label="变更类型">
              <el-select
                ...
                :disabled="false"
              >
                <el-option label="简单变更" value="简单变更"></el-option>
                <el-option label="标准变更" value="标准变更"></el-option>
                <el-option label="紧急变更" value="紧急变更"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
      ...
      <div class="dealStepsArea">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="下一环节期望完成时间" prop="nextLinkExpectedCompleteTime">
              <el-date-picker
                class="oms-input"
                type="datetime"
                placeholder="选择日期"
      ...
      <el-tabs type="border-card" class="tabsArea">
        <el-tab-pane label="计划与方案">
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item label="制定计划开始时间" prop="formulatePlanStartTime">
                <el-date-picker
                  class="oms-input"
                  type="datetime"
                  placeholder="选择日期"
              ...
              <el-form-item label="制定计划结束时间" prop="formulatePlanEndTime">
                <el-date-picker
                  class="oms-input"
                  type="datetime"
                  placeholder="选择日期"
              ...
              <el-form-item label="实施计划开始时间" prop="implementationPlanStartTime">
                <el-date-picker
                  class="oms-input"
                  type="datetime"
                  placeholder="选择日期"
              ...
              <el-form-item label="实施计划结束时间" prop="implementationPlanEndTime">
                <el-date-picker
                  class="oms-input"
                  type="datetime"
                  placeholder="选择日期"
  ...
  methods: {
    ...
    queryData() {
      ...
      service.queryData(params).then(res => {
        ...
        if (!this.isDraft) {
          ...
          this.ruleForm.isUpdateConfiguration = "";
          this.ruleForm.isInterruptBusiness = "";
    ...
    validatePass(rule, value, callback) {
      if ((!value || value.length < 1) && rule.fullField !== "message") {
        ...
        } else if (rule.fullField === "formulatePlanEndTime") {
          if (
            new Date(value).getTime() <
            new Date(this.ruleForm.formulatePlanStartTime).getTime()
          ) {
            callback(new Error(" "));
            this.$notify({
              title: "提示",
              message: "制定计划结束时间不能小于制定计划开始时间",
              duration: 2000
            });
          } else {
            callback();
          }
        } else if (rule.fullField === "implementationPlanStartTime") {
          // 实施计划开始时间 必须大于 制定计划结束时间
          if (
            new Date(value).getTime() <
            new Date(this.ruleForm.formulatePlanEndTime).getTime()
          ) {
            callback(new Error(" "));
            this.$notify({
              title: "提示",
              message: "实施计划开始时间不能小于制定计划结束时间",
              duration: 2000
            });
          } else {
            callback();
          }
        } else if (rule.fullField === "implementationPlanEndTime") {
          if (
            new Date(value).getTime() <
            new Date(this.ruleForm.implementationPlanStartTime).getTime()
          ) {
            callback(new Error(" "));
            this.$notify({
              title: "提示",
              message: "实施计划结束时间不能小于实施计划开始时间",
              duration: 2000
            });
          } else {
            callback();
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          switch (ruleForm.alterType) {
            case "简单变更":
              data.variables.push({ key: "changeType", value: "simple" });
              break;
            case "标准变更":
              data.variables.push(
                { key: "changeType", value: "standard" },
                {
                  key: "managerUserId",
                  value: this.manager
                }
...
.makePlan /deep/{
  ...
  .multi-cascader .el-multicascader__label {
    margin: 0;
