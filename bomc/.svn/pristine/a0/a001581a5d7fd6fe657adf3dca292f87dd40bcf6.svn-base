    ...
    >
      ...
      <div class="basicInfoArea">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="重复事件标记：" prop="isRepeating">
              <el-select v-model="ruleForm.isRepeating" class="oms-select">
      ...
      <div class="dealStepsArea">
        <el-row>
          <el-col :span="8">
            <el-form-item label="是否解决：" prop="checkResult">
              <el-select v-model="ruleForm.checkResult" class="oms-select">
                <el-option label="解决" value="1"/>
                <el-option label="未解决" value="0"/>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8" v-if="ruleForm.checkResult==='0'">
            <el-form-item label="下一环节解决人" prop="nextNodeDealMan">
              <el-cascader
                class="oms-cascader"
                :show-all-levels="false"
                :options="nextNodeDealManList"
                v-model="ruleForm.nextNodeDealMan"
                @change="nextNodeDealManChange"
              ></el-cascader>
              <!-- <el-select v-model="ruleForm.nextNodeDealMan" class="oms-select">
                <el-option v-for="(item,index) in nextNodeDealManList" :key="index" :label="item.realName" :value="item.userId" />
              </el-select>-->
            </el-form-item>
          </el-col>
        </el-row>
      ...
      <el-tabs type="border-card" class="tabsArea">
        <el-tab-pane label="解决方案">
          <el-row>
            <el-col :span="24">
              <el-form-item
                class="dealSuggestion textarea"
                label="解决方案："
                prop="fullMsg"
                v-if="fullMsgShow"
              >
                <span class="inputTips">最多输入500个字符</span>
                <el-input type="textarea" v-model="ruleForm.fullMsg" class="oms-input"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
        </el-tab-pane>
      ...
      <el-row class="operation">
        <el-button type="info">转变更流程</el-button>
        <el-button type="info" @click="regressesQuery">回退</el-button>
...
export default {
  ...
  watch: {
    ...
    "ruleForm.checkResult": function(val, oldVal) {
      if (val === "0") {
        //0代表未解决
        this.fullMsgShow = false;
        this.ruleForm = { ...this.ruleForm, fullMsg: "" };
      } else if (val === "1") {
        this.fullMsgShow = true;
      }
    }
  },
  ...
  methods: {
    ...
    queryData() {
      ...
      if (this.isDraft) {
        params.isDraft = 1;
        this.queryComments(); // 如果是从草稿箱进来查询处理意见
      }
      ...
      service.queryData(params).then(res => {
        ...
        this.ruleForm.eventStatus = "一/二线处理";
        ...
        // console.log(this.ruleForm.relatedOrg)
        ...
        this.serarchNextNodeDealMan(); //拿到机构id查询下一环节解决人
      });
    },
    queryComments() {
      const params = {
      ...
      service.queryComments(params).then(res => {
        ...
        this.ruleForm.fullMsg = res.result[0].fullMsg;
        this.commentId = res.result[0].id;
        if (res.result[0].assignee !== null) {
          this.ruleForm.nextNodeDealMan = res.result[0].assignee.split(",");
        }
      });
    },
    queryAttachments() {
    ...
    saveAsDraft(formName) {
      ...
      if (this.ruleForm.checkResult == "1") {
        // 已解决
        dataComments.fullMsg = this.ruleForm.fullMsg;
      } else if (this.ruleForm.checkResult == "0") {
        // 未解决
        dataComments.assignee = this.ruleForm.nextNodeDealMan.join();
      }
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          ruleForm.checkResult = parseFloat(ruleForm.checkResult);

          if (ruleForm.nextNodeDealMan) {
            ruleForm.nextNodeDealMan =
              ruleForm.nextNodeDealMan[ruleForm.nextNodeDealMan.length - 1] /
              -10000;
          }
          ...
          if (this.ruleForm.checkResult == 0) {
            //未解决
            data.variables = [
              { key: "resolve", value: "false" },
              { key: "userIdFourth", value: ruleForm.nextNodeDealMan }
            ];
          } else {
            //已解决
            data.variables = [{ key: "resolve", value: "true" }];
          }
    ...
    serarchNextNodeDealMan() {
      ...
      service.serarchNextNodeDealMan().then(res => {
        let deep = function(arr) {
          arr.forEach(item => {
            item.value = item.object.deptId;
            item.label = item.object.deptName;
    ...
    validatePass(rule, value, callback) {
      ...
      } else if (rule.fullField === "fullMsg") {
        if (value && !this.getStrLenght(value, 500)) {
          callback(new Error(" "));
          this.$notify({
            title: "提示",
