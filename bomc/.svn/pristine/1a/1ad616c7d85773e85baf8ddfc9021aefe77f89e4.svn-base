<template>
  <div class="oms-diy-elementui eventContainer problemContainer changeForm taskImplement">
    ...
    >
        ...
        <el-tab-pane label="计划与方案">
          <el-row>
            ...
            <el-col :span="12">
              ...
              <el-form-item label="实际实施开始时间" prop="planStartTime">
                <el-date-picker
                  class="oms-input"
                  type="datetime"
                  placeholder="选择日期"
              ...
              <el-form-item label="实际实施结束时间" prop="planEndTime">
                <el-date-picker
                  class="oms-input"
                  type="datetime"
                  placeholder="选择日期"
      ...
      <el-row class="operation">
        <el-button type="info">触发发布流程</el-button>
    ...
    validatePass(rule, value, callback) {
      if ((!value || value.length < 1) && rule.fullField !== "message") {
        ...
        if (nextLinkTime > createTime && nextLinkTime < dueTime) {
          ...
          this.$notify({
            title: "提示",
            message: "下一环节期望完成时间必须大于登记时间并且小于期望解决时间",
            duration: 2000
          });
        }
      } else if (rule.fullField === "planStartTime") {
        if (
          ...
          this.$notify({
            title: "提示",
            message: "实际实施开始时间不能小于实施计划结束时间",
            duration: 2000
          });
        } else {
          callback();
        }
      } else if (rule.fullField === "planEndTime") {
        if (
          new Date(value).getTime() <
          new Date(this.ruleForm.planStartTime).getTime()
        ) {
          callback(new Error(" "));
          this.$notify({
            title: "提示",
            message: "实际实施结束时间不能小于实际实施开始时间",
            duration: 2000
          });
        } else {
          callback();
      ...
      if (this.done) {
        this.$refs[formName].validate(valid => {
          if (valid) {
            ...
            ruleForm.noticeDepartment = JSON.stringify(
              ruleForm.noticeDepartment
            );
            ruleForm.affectedApplicationSystem = JSON.stringify(
              ruleForm.affectedApplicationSystem
            );
