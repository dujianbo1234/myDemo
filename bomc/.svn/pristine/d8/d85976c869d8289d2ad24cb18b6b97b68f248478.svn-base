<template>
  <div class="oms-diy-elementui eventContainer problemContainer changeForm urgClose">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="关闭人" prop="closeUserId">
              <el-input v-model="closeUserName" class="oms-input" :disabled="true"></el-input>
              <el-input v-show="false" v-model="ruleForm.closeUserId" class="oms-input"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
  ...
  methods: {
    ...
    queryData() {
      ...
      service.queryData(params).then(res => {
        ...
        this.ruleForm.closeUserId = this.$store.getters.userId;
        if (!this.isDraft) {
    ...
    validatePass(rule, value, callback) {
      if ((!value || value.length < 1) && rule.fullField !== "message") {
        ...
        } else if (rule.fullField === "closeTime") {
          let createTime = new Date(this.ruleForm.createTime).getTime();
          let closeTime = new Date(this.ruleForm.closeTime).getTime();
          if (closeTime < createTime) {
            callback(new Error(" "));
            this.$notify({
              title: "提示",
              message: "关闭时间不能小于登记时间",
              duration: 2000
            });
          } else {
            callback();
