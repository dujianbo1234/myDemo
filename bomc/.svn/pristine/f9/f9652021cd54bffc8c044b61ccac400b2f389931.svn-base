<template>
  <div class="oms-diy-elementui eventContainer">
    ...
    >
      ...
      <div class="basicInfoArea">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="事件优先级：" prop>
              <el-select v-model="ruleForm.eventPriority" placeholder class="oms-select">
                <el-option
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="事件影响度：" prop>
              <el-select v-model="ruleForm.eventInfluenceDegree" class="oms-select">
                <el-option
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="协查原因类别：" prop="consultationReasonCategory">
              <el-select v-model="ruleForm.consultationReasonCategory" class="oms-select">
                <el-option
          ...
          <el-col :span="8">
            <el-form-item class="pointer" label="所属系统信息：" prop>
              <el-cascader v-model="ruleForm.system" :options="systemInfo" :disabled="true"></el-cascader>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
      ...
      <div class="dealStepsArea">
        <process-steps :procInstId.sync="procInstId" @getArrivedTime="getArrivedTime"></process-steps>
      </div>
      ...
      <div class="dealStepsArea">
        <el-row>
          <el-col :span="8" v-if="ruleForm.eventPriority != '紧急'">
      ...
      <el-row class="operation">
        ...
        <!-- <el-button type="info">升级</el-button> -->
    ...
    <div v-show=" tabActive === 'chart' " class="flow-chart">
      <img v-if="tabActive" :src="flowChartData" class="chart" alt="流程图" @click="enlarge">
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    ...
    arrivedTime: function(val) {
      // 当拿到到达时间，才去查询表单数据，因为事件完成期限依赖于到达时间进行计算
      if (val !== "") {
        this.queryData();
      }
    }
  ...
  methods: {
    ...
    calculateDeadTime() {
      ...
      var timeStamps =
        parseFloat(new Date(this.arrivedTime).getTime()) +
    ...
    queryData() {
      ...
      service.queryData(params).then(res => {
        ...
        this.ruleForm.eventStatus = "紧急事件确认/转派";

        if (res.result[0].startMode === "0") {
          // 0 代编自动提单
          this.automaticSheet = true;
        }
        ...
        //   this.ruleForm.relatedOrg[index] = this.ruleForm.relatedOrg[index] + ""
        // });
      ...
      this.$confirm("确认退回到上一个节点吗？", "提示", {
        ...
        .then(() => {
          let data = {
            procInstId: this.procInstId,
            taskId: this.taskId,
            taskKey: "sid-201810211055613",
            assignee: this.userId,
            status: 1,
            data: JSON.stringify({ form: { message: this.ruleForm.message } })
          };
          if (this.automaticSheet) {
            data.taskKey = "sid-201810211055615";
          }
          service
            .nodeBack(data)
            .then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (ruleForm.eventPriority === "紧急") {
            data.variables = [{ key: "urgent", value: "true" }];
          } else {
            data.variables = [
              { key: "urgent", value: "false" },
              { key: "userIdThird", value: this.ruleForm.nextNodeDealMan }
            ];
          }
    ...
    getArrivedTime(value) {
      this.arrivedTime = value;
      console.log("arrivedTime: ", this.arrivedTime);
    }
  }
};
</script>
