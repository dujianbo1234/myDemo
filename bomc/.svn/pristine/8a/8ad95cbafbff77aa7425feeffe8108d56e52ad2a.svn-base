    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      <el-container>
        <el-header class="el_head">
          ...
          <span class="model">所属模型：<strong>{{this.$store.state.resourceManagement.name}}</strong></span>
        </el-header>
        <el-main class="el_main">
              ...
              <div class="content_row">
                <div class="content_col">
                  <el-form-item label="搜索代码" prop="ciid">
                    <!-- <el-input v-model="ruleForm.ciid" placeholder="请输入搜索代码" size="mini"></el-input> -->
                    <span class="search_code">{{searchCode}}</span>
                    <el-input class="code" v-model="ruleForm.ciid" placeholder="请输入搜索代码" size="mini"></el-input>
                  </el-form-item>
                </div>
                <div class="content_col">
              ...
              <div class="content_row">
                <div class="content_col">
                  ...
                  <el-form-item label="状态" v-if="levelClass != '物理类'">
                    <el-input  v-model="ruleForm.state" size="mini" disabled="disabled"></el-input>
                  </el-form-item>
                </div>
                <div class="content_col">
              ...
              <div class="content_row">
                <div class="content_col">
                  <el-form-item label="所属系统" prop="name">
                    <el-select v-model="ruleForm.name" disabled="disabled" size="mini">
              ...
              <div class="content_row">
                <div class="content_col">
                  <el-form-item label="影响客户" >
                    <el-select v-model="ruleForm.influence_customer" placeholder="请选择影响客户" size="mini">
          ...
          <div class="content_head">
            ...
            <div class="content_main">
              <div class="content_row" v-for="(item,index) in ruleForm.relationship" :key="index">
                  <div class="content_col" style="width:24%">
                    <el-form-item label="发起端资源">
                      <el-input v-model="item.source"  size="mini" :disabled = true></el-input>
                    </el-form-item>
                  </div>
                  <div class="content_col">
                    <el-form-item label="节点关系" :prop="'relationship.' + index + '.relation'" 
                    :rules="[{ required: true, message: '节点关系不能为空', trigger: 'change'}]">
                      <el-select v-model="ruleForm.relationship[index].relation" placeholder="请选择节点" @change="choiceRelation(index,ruleForm.relationship[index].relation)" size="mini">
                        <el-option
                  ...
                  <div class="content_col">
                    <el-form-item label="接收端资源" :prop="'relationship.' + index + '.target'" 
                    :rules="[{ required: true, message: '接收端资源不能为空', trigger: 'change'}]">
                      <el-select v-model="ruleForm.relationship[index].target" placeholder="请选择接收端资源" size="mini" :disabled = item.show>
                        <el-option
                          v-for="(item,index) in ruleForm.relationshipArr[index].target"
                          :key="index"
                          :value="item.ciid">
                        </el-option>
                      </el-select>
...
export default {
  ...
  created() {
    if (!this.$store.state.resourceManagement.name) {
      debugger;
      this.$router.push({ path: "/resourceData/data" });
      return;
    } else {
      debugger;
      this.attProprietary(this.$store.state.resourceManagement.name);
      this.ruleForm.level = this.$store.state.resourceManagement.level;
      this.ruleForm.model = this.$store.state.resourceManagement.name;
      service
        .queryRelationship(this.$store.state.resourceManagement.name)
        .then(res => {
          debugger;
          if (res) {
            if (!res.datas) {
      ...
      temp.system = this.ruleForm.name;
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        // if (valid) {
          this.$refs["Proprietary"].validate(validAttr => {
            debugger;
            if (valid && validAttr) {
              debugger;
              ...
              obj.资源关系 = [];
              this.ruleForm.relationship.map(function(item) {
                var temp = [];
                temp.push(item.source, item.relation, item.target);
                obj.资源关系.push(temp);
              });
              obj.ciid = this.searchCode + obj.ciid;
              delete obj.relationship;
              delete obj.relationshipArr;
              console.log(JSON.stringify(obj))
              service.abbResourcesData(obj).then(res => {
                debugger;
                console.log(res);
                if (res.type == 0) {
                  this.URL = false;
    ...
    attProprietary(data) {
      ...
      service.attProprietary(data).then(res => {
        debugger;
        console.log(JSON.stringify(res.datas[0].attributes));
    ...
    addNode() {
      debugger;
      this.ruleForm.relationship.push({
        source: "系统自动填写",
        relation: "",
        target: "",
        show: true
      });
      this.ruleForm.relationshipArr.push({
        source: "",
        relation: "",
        target: ""
      });
    ...
    choiceRelation(index, item) {
      debugger;
      this.ruleForm.relationship[index].relation = item.name;
      this.ruleForm.relationship[index].show = false;
      service.queryReceive(item.target).then(res => {
        debugger;
        if (res.count === "0") {
          this.ruleForm.relationshipArr[index].target = "";
          this.ruleForm.relationship[index].target = "";
        } else {
          this.ruleForm.relationshipArr[index].target = res.datas[item.target];
        }
      });
    },
    // 删除节点关系
    removeDomain(item) {
      debugger;
      var index = this.ruleForm.relationship.indexOf(item);
      if (index !== -1) {
        this.ruleForm.relationship.splice(index, 1);
        this.ruleForm.relationshipArr.splice(index, 1);
      }
    },
    open2(formName) {
