        ...
        <div>
          <el-form :rules="rules" :inline="true" :model="selectData" label-position="right" label-width="90px" size="small" ref="qForm" style="text-align:left;">
            ...
            <div class="logAdd-obj" v-if="collectType !== 0">
              ...
              <el-form-item label="实例:" prop="sid" v-if="showSid&&collectType===4">
                <el-input v-model="selectData.sid"></el-input>
              </el-form-item>
            </div>
          </el-form>
        </div>
      ...
      <div class="details-title" v-if="collectType !== 0">
        ...
        <div class="details-obj">
          <el-form :inline="true" :model="selectData" label-position="right" label-width="75px" size="mini" style="text-align:left;">
            ...
            <el-form-item label="实例:" v-if="showSid && collectType===4">
              {{selectData.sid}}
            </el-form-item>
          </el-form>
        </div>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.showSid = this.resource_type[3] === 'Oracle' ? true : false;
      if(this.optType === 'add'){
    ...
    methods: {
      ...
      resChange(value) {
        this.showSid = value[3] === 'Oracle' ? true :false;
    ...
    update_save(data) {
      ...
      if (bloo) {
        ...
        if((data.file.indexOf('%i') > 0 || (data.historyFile.indexOf('%i')>0)) && data.initFileNum === ''){
          this.detailData.initFileNum = 0;
        }
        delete this.detailData.log;
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.selectData.template_content.push({
              ...
              initFileNum: data.initFileNum | number,
              filterContentRule: data.filterContentRule,
