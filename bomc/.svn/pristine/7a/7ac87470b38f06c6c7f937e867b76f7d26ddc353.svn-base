<template>
  <div v-if="show">
    <el-form label-position="top">
      ...
      <el-form-item label="占位内容" v-if="Object.keys(data.options).indexOf('placeholder')>=0 && (data.type!='time' || data.type!='date' || data.type!='table')">
      ...
      <el-form-item label="数据源配置" v-if="Object.keys(data.options).indexOf('sourceData')>=0 && data.type != 'table'">
         <el-select
        v-model="data.options.sourceApi"
      ...
      >
        <el-option v-for="item in chartsSource" :key="item.key" :value="item.value" :label="item.label"></el-option>
      </el-select>
      </el-form-item>
      <el-form-item label="表格数据源配置" v-if="Object.keys(data.options).indexOf('sourceData')>=0 && data.type =='table'">
         <el-select
        v-model="data.options.sourceApi"
        :disabled="data.options.disabled"
        :multiple="data.options.multiple"
        ...
        :style="{width: data.options.width}"
      >
        <el-option v-for="item in tableSource" :key="item.key" :value="item.value" :label="item.label"></el-option>
      </el-select>
      </el-form-item>
      <!-- <el-form-item label="表格数据" v-if="data.type == 'table' && Object.keys(data.options).indexOf('data')>=0 ">
        表格数据
        <el-button type="text" @click="dialogTableVisible = true">编辑表格数据</el-button>
        <el-dialog title="数据编辑" :visible.sync="dialogTableVisible">
              <div>
                <el-button>添加行</el-button>
                <el-button>添加列</el-button>
              </div>
              <el-table :data="data.options.data">
                <el-table-column v-for="item in data.options.options" :prop="item.prop" :label="item.label"></el-table-column>
              </el-table>
            </el-dialog>
      </el-form-item> -->
      ...
      <template v-if="data.type != 'grid' && data.type != 'lineChart'">
...
export default {
  ...
  data () {
    return {
      ...
      chartsSource: [{key:'1',value:'/bar1',label:'柱状图数据源1'},{key:'2',value:'/bar2',label:'柱状图数据源2'},{key:'3',value:'/bar3',label:'柱状图数据源3'},{key:'4',value:'/line1',label:'折线图数据源1'},{key:'5',value:'/pie1',label:'饼图数据源1'},{key:'6',value:'/projectGradientLine2',label:'圆柱状数据源1'}],
      tableSource: [{key:'1',value:'/table1',label:'数据源1'}]
    }
  },
  mounted() {
   
    if(this.data && Object.keys(this.data.options).indexOf('sourceData')>=0){
    }
  },
  computed: {
    show () {
  ...
  methods: {
    sourceChange(res){
      ...
      this.data.options.sourceApi = res
    },
    ...
    editData(obj) {
      let _this = this;
      this.tableData = JSON.parse(JSON.stringify(obj));
      let template = '';
      // for(let item in obj){
      //   template += '<el-tag type="success">'+obj[item].d1+'</el-tag>'
      // }
     template =  `<div>{{tableData}}</div><el-table
              data="tableData"
              style="width: 100%">
