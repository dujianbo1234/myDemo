						...
						<el-col>
							<el-card>
										...
										<el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline">
											<el-form-item label="岗位名字" prop="postName">
												<el-input v-model="formInline.postName" style="width: 210px;"></el-input>
											</el-form-item>
...
<!--<el-button class="opt_btn" @click="beback2()">恢复</el-button>-->
								...
								<el-table :data="tableData.slice((currentPage-1)*pagesize,currentPage*pagesize)" :default-sort="{order: 'ascending'}" @selection-change="handleSelectionChange" @select-all="selectAllRus">
									...
									<el-table-column label="岗位名字" prop='postName' width="100">
									</el-table-column>
		...
		<i class="iconfont co" title="禁用"  @click="handleDelete(scope.row.postId,1)">&#xe65a;</i> 
		<i class="iconfont co" title="恢复" @click="beback(scope.row.postId)">&#xe65c;</i>
								</template>
		</el-table-column>
	    ...
	    <el-button class="form_btn" size="mini"   @click="handleDelete(scope.row.postId,2)">删除</el-button>
		...
		<!--新增 -->
		<el-dialog title="新增岗位" :visible.sync="addUserDialog" :before-close="handleClose">
			<!--<el-form :model="formAdd">-->
			<el-form :model="formAdd" :rules="rules" ref="formAdd" style="margin:10px;width:auto;">


				<el-form-item label="岗位名:" :label-width="formLabelWidth" prop="postName" required>
					<el-input v-model.trim="formAdd.postName" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="岗位方法:" :label-width="formLabelWidth" prop="method" required>
					<el-input v-model.trim="formAdd.method" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="岗位描述:" :label-width="formLabelWidth" prop="remark" required>
		...
		<!--03-编辑-->
		<el-dialog title="编辑岗位" :visible.sync="resetDialog22" :before-close="handleClose">
			<el-form :model="formEd" :rules="rules" ref="formEd" style="margin:10px;width:auto;">

				<!--<el-form-item label="岗位键值" :label-width="formLabelWidth">
				<el-input v-model.trim="formEd.postKey" auto-complete="off"></el-input>
			</el-form-item>-->
				<el-form-item label="岗位名字:" :label-width="formLabelWidth" prop="postName" required>
					<el-input v-model.trim="formEd.postName" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="岗位描述:" :label-width="formLabelWidth" prop="remark" required>
...
<script>
	...
	import { roleSch, rightDele, recover, adCmit, editCmit } from "../../../api/postManage";
	export default {
		data() {
			return {
					// 新增的表单数据
				...
				rules: {
					//					postKey: [
					//						{ required: true, message: '请输入岗位键值', trigger: 'blur' },
					...
					method: [
						{ required: true, message: '请输入岗位方法', trigger: 'blur' },
						{ min: 3, max: 15, message: '长度在 3 到 15 个字符', trigger: 'blur' }
					],
					remark: [
		...
		watch: {
			...
			//					this.arrRo.push(this.multipleSelection[i].postId);
			//				}
			//			}
		},
			...
			exportLogo() {
				if(this.multipleSelection == undefined || this.multipleSelection.length == 0) {
					import('@/modules/mms/vendor/Export2Excel').then(excel => {
						...
						const filterVal = [       
							'postId', 'postName', 'createTime', 'remark', 
						]					
