	...
	<div>
		<el-table :data="data" border style="width: 100%" :row-style="showTr">
          ...
          <i v-if="scope.row._expanded" class="el-icon-arrow-down" aria-hidden="true"></i>
				...
				<span v-if="spaceIconShow(index)" style="margin-left: 10px;" v-for="(space, levelIndex) in scope.row._level" class="ms-tree-space"></span>
				<button class="button is-outlined is-primary is-small" style="width: 24px;height: 20px;background: white;border: none;" v-if="toggleIconShow(index,scope.row)" @click="toggle(scope.$index,scope.row.id)">
          <i v-if="!scope.row._expanded" class="el-icon-caret-right" aria-hidden="true"></i>
          <i v-if="scope.row._expanded" class="el-icon-caret-bottom" aria-hidden="true"></i>
        </button>
				<!--<span v-else-if="index===0" class="ms-tree-space"></span>--> 
				
				<span v-else-if="index===0" class="ms-tree-space" style="font-weight: 10px;line-height:12px;width:24px;height:20px;">
		<button class="button is-outlined is-primary is-small" style="width: 24px;height: 20px;background: white;border: none;"   @click="toggle(scope.$index,scope.row.groupId)">
