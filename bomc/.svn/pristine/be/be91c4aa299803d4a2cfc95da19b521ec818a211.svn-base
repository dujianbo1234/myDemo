    ...
    submitForm(formName) {
      ...
      if (this.done) {
        this.$refs[formName].validate(valid => {
          if (valid) {
            this.ruleForm.closeTime = parseTime(
              Date.now(),
              "{y}-{m}-{d} {h}:{i}:{s}"
            );
            let ruleForm = Object.assign({}, this.ruleForm);
            ...
            this.$confirm("是否确认提交?", "提示", {
              ...
              .then(async () => {
                ...
                Promise.all(postArr)
                  .then(res => {
                    service.modifyProcess(this.procInstId).then(res => {
                      loading.close();
                      this.$message({
                        message: "关闭成功！",
                        type: "success"
